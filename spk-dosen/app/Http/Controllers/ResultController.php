...
class ResultController extends Controller
{
    ...
    public function calculate(Request $request) {
        ...
        foreach ($students as $i => $value) {
            ...
            if($value->lulusan = 'Swasta / S2') {
                $c2[$i] = 6;
            }
            if($value->lulusan = 'Swasta / S3') {
                $c2[$i] = 9;
            }
            if($value->lulusan = 'Negeri / S2') {
                $c2[$i] = 6;
            }
            if($value->lulusan = 'Negeri / S3') {
                $c2[$i] = 9;
            }
        }
       
        foreach ($students as $i => $value) {
            if($value->pengalaman_mengajar <= 0) {
                $c4[$i] = 1;
            }
            if($value->pengalaman_mengajar >= 1 && $value->pengalaman_mengajar <= 5) {
                $c4[$i] = 3;
            }
            if($value->pengalaman_mengajar >= 6 && $value->pengalaman_mengajar <= 10) {
                $c4[$i] = 5;
            }
            if($value->pengalaman_mengajar >= 11 && $value->pengalaman_mengajar <= 20) {
                $c4[$i] = 7;
            }
            if($value->pengalaman_mengajar > 20) {
                $c4[$i] = 9;
            }
        }
        ...
        foreach ($alternatif as $i => $value) {
            $performance_score[$i] = round($c1_V[$i] * $c2_V[$i] *  $c4_V[$i], 2);
        }
